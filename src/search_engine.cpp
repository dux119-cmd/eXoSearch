...
[[nodiscard]] int SearchEngine::score(const Entry& entry, const std::string_view query) const
{
    if (query.empty()) {
...
[[nodiscard]] std::vector<std::string> SearchEngine::find_completions(const std::string_view query) const
{
